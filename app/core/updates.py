...
from app.core.identity import APP_DISPLAY_NAME, APP_EXE_NAME, APP_SLUG, APP_VERSION, APP_ZIP_NAME, GITHUB_REPO, LOG_NAME
from app.core.paths import is_frozen

logger = logging.getLogger(f'{LOG_NAME}.updates')
    ...
    try:
        with zipfile.ZipFile(zip_path, 'r') as archive:
            ...
            if not names:
                return None
            preferred = next(
                (n for n in names if Path(n).name.lower() in {APP_EXE_NAME.lower(), 'aimsync.exe'}),
                names[0],
            )
            dest = updates_dir() / f'{APP_SLUG}-new.exe'
...
def download_update(download_url: str, *, asset_name: str = '') -> tuple[Path | None, str | None]:
    ...
    name = asset_name or Path(download_url).name or f'{APP_SLUG}-download'
    ...
    if dest.suffix.lower() == '.zip':
        ...
        if extracted is None:
            return None, f'Zip did not contain {APP_EXE_NAME}'
...
def _spawn_updater(new_exe: Path, target_exe: Path) -> tuple[bool, str]:
    ...
    return True, f'Updater scheduled — {APP_DISPLAY_NAME} will restart'
...
def install_update(*, repo: str = DEFAULT_REPO) -> dict[str, Any]:
    if not is_frozen():
        return {
            'success': False,
            'message': f'In-app updates apply to the built {APP_EXE_NAME} only. Download the latest release manually.',
        }
    ...
    staged = updates_dir() / f'{APP_SLUG}-{status.get("latest_version", "new")}.exe'
    if new_exe != staged:
