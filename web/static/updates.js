...
(function (global) {
    ...
    const shellCfg = () => global.AIMSYNC_SHELL || {};

    function installAppUpdate() {
        const exeName = shellCfg().exeName || 'AimSyncCS2Makcu.exe';
        if (!confirm(`Download and install the latest ${exeName}? The app will close and restart.`)) return;
        const installBtn = document.getElementById('update-install-btn');
        const footerBtn = document.getElementById('footer-update-btn');
        const msg = document.getElementById('update-status-msg');
