...
from app.core.env import env_flag, env_int
from app.core.identity import APP_DISPLAY_NAME, APP_EXE_NAME, APP_STORAGE_DIR, APP_VERSION, APP_VERSION_LABEL, GITHUB_REPO
from app.core.paths import web_root
from app.core.runtime import recoil_only_enabled
from app.recoil.weapon_data import WeaponData
from app.recoil.weapon_icons import WEAPON_ICON_FILES
...
def _is_docker_bridge_ip(ip: str) -> bool:
    ...
    try:
        ...
        return False
    return parts[0] == '172' and 16 <= second <= 31


def get_local_ip() -> str:
    from app.core.env import env_get

    override = env_get('LAN_IP')
    if override:
        return override
    ...
    if sys.platform.startswith('linux'):
        try:
            ...
            for candidate in out.split():
                if candidate and not candidate.startswith('127.') and not _is_docker_bridge_ip(candidate):
                    return candidate
...
def create_app() -> Flask:
    ...
    def index():
        ...
        return render_template(
            ...
            weapon_icon_files=WEAPON_ICON_FILES,
            recoil_only=recoil_only,
        )
    ...
    def quit_app():
        config = load_config()
        should_shutdown_pc = bool(config.get('shutdown_on_app_stop', False)) and sys.platform == 'win32'
        ...
        def shutdown_sequence() -> None:
            ...
            if _shutdown_callback is not None:
                try:
                    ...
                    pass
            if should_shutdown_pc and sys.platform == 'win32':
                try:
...
def run_flask() -> None:
    global _server
    app = create_app()
    _server = make_server('0.0.0.0', flask_port(), app)
