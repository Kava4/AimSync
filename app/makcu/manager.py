"""Makcu bridge — hardware production path + optional Windows-only software fallback."""
...
class MakcuManager:
    ...
    def set_dev_allowed(cls, allowed: bool) -> None:
        # SendInput fallback is Windows-only.
        cls._dev_allowed = bool(allowed) and sys.platform == 'win32'
    ...
    def connect():
        ...
        with MakcuManager.connection_lock:
            ...
            try:
                ...
                device = env_get('MAKCU_DEVICE')
                if device:
                    try:
                        MakcuManager.controller = create_controller(port=device, **kwargs)
                    except TypeError:
                        MakcuManager.controller = create_controller(**kwargs)
                else:
                    MakcuManager.controller = create_controller(**kwargs)
                ...
                MakcuManager.device_port = str(port) if port else 'USB'
                ...
                if MakcuManager._dev_allowed and sys.platform == 'win32':
                    MakcuManager.dev_mode = True
                    print('[MAKCU] Dev mode: using local mouse (SendInput) — not for dual-PC')
                    return object()
                if sys.platform != 'win32':
                    print('[MAKCU] Hardware required on this platform (no Win32 fallback)')
                return None
            return MakcuManager.controller
    ...
    def move_relative(dx: float, dy: float) -> bool:
        if MakcuManager.dev_mode:
            ...
            return True
        if not MakcuManager.is_hardware():
            return False
        if dx == 0 and dy == 0:
            return False
        try:
